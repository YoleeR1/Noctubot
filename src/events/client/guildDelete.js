...
// Filter out any schema files that don't exist to prevent errors
const schemas = schemaFiles.map(file => {
    try {
        return require(`../../database/models/${file}`);
    } catch (error) {
        console.log(`Warning: Could not load schema file ${file}: ${error.message}`);
        return null;
    }
