...
module.exports = async (client) => {
    ...
    client.loadSubcommands = async function (client, interaction, args) {
        try {
            ...
            // Build the file path
            const commandsPath = path.join(process.cwd(), 'src', 'commands', interaction.commandName);
            const subcommandName = interaction.options.getSubcommand();
            const fileName = data?.Beta ? `${subcommandName}-beta` : subcommandName;
            const fullPath = path.join(commandsPath, `${fileName}.js`);

            try {
                // Try dynamic import first (for ESM)
                const command = await import(fullPath).catch(async () => {
                    // Fallback to require (for CommonJS)
                    return require(fullPath);
                });

                // Execute the command
                const commandFunction = command.default || command;
                return commandFunction(client, interaction, args).catch(err => {
                    client.emit("errorCreate", err, interaction.commandName, interaction);
                });
            } catch (err) {
                console.error(`Failed to load subcommand: ${fullPath}`, err);
                client.emit("errorCreate", err, interaction.commandName, interaction);
            }
        } catch (err) {
            console.error('Error in loadSubcommands:', err);
            client.emit("errorCreate", err, interaction.commandName, interaction);
        }
    }
